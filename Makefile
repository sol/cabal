...
.PHONY: outdated
outdated:
	# If you temporarily disable `cabal outdated` for a package:
	#
	# - on `master`, then open a corresponding issue to reenable it:
	#   https://github.com/haskell/cabal/issues/new?labels=priority:+high+:fire:&title=reenable+%60cabal+bounds%60+on+CI
	# - on a release branch, then make sure that you run `make outdated`
	#   manually before making a release
	#
	# NOTE: To "disable" `cabal outdated` for a package, only comment out
